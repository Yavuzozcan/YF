...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function openTx(e){e?.preventDefault();if(!txModal||!txForm)return alert("İşlem formu bulunamadı.");txForm.reset();fillTxCards();if(txDate)txDate.value=new Date().toISOString().split("T")[0];toggleTxCard();txModal.classList.remove("hidden");document.body.style.overflow="hidden"}
  function closeTx(){txModal.classList.add("hidden");document.body.style.overflow=""}
  document.querySelectorAll("#openTransactionFormButton").forEach(button=>button.addEventListener("click",openTx));
  document.querySelectorAll("#closeTransactionFormButton").forEach(button=>button.addEventListener("click",closeTx));
  document.querySelectorAll("#cancelTransactionButton").forEach(button=>button.addEventListener("click",closeTx));
...
// =========================================
// YF v4.9 — BORÇ ÖDE MASTER FIX
// v4.7 / v4.8 kullanılmaz.
// Kart ve krediler gerçek ID ile tamamen ayrılır.
// =========================================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  rebuildDebtPicker();
  refreshPaymentSystem();

  document.addEventListener("click", event => {
    const pageButton = event.target.closest('[data-page="debtPaymentPage"]');

    if (pageButton) {
      setTimeout(refreshPaymentSystem, 80);
      setTimeout(refreshPaymentSystem, 300);
    }
  });

  window.addEventListener("pageshow", refreshPaymentSystem);
  window.addEventListener("storage", refreshPaymentSystem);

  // -------------------------------------------------
  // ESKİ BORÇ ÖDE EVENTLERİNİ TEMİZLE
  // -------------------------------------------------
  function resetDebtPaymentForm() {
    const oldForm = $("debtPaymentForm");
    if (!oldForm) return;

    const newForm = oldForm.cloneNode(true);
    oldForm.replaceWith(newForm);

    const select = newForm.querySelector("#debtPaymentCard");
    const amount = newForm.querySelector("#debtPaymentAmount");

    if (!select || !amount) return;

    select.classList.remove("hidden");

    select.addEventListener("change", updatePaymentPanel);
  ...
  // -------------------------------------------------
  // BORÇ LİSTESİ — TAMAMEN ID
  // -------------------------------------------------
  function refreshPaymentSystem() {
    const select = $("debtPaymentCard");
    if (!select) return;
    ...
    debts
      ...
      .forEach(item => {
        ...
        option.dataset.debtType = item.type || "credit-card";
    ...
    if (
      oldValue &&
      debts.some(item => String(item.id) === String(oldValue))
    ) {
      select.value = oldValue;
    }
  ...
  function updatePaymentPanel() {
    ...
    const selected = loadJson(DEBT_KEY).find(
    ...
    const smartPanel = $("smartMinimumPanel");
    const summaryTitle = document.querySelector(
      "#debtPaymentPage .debt-payment-summary span"
    );
    ...
    if (selected.type === "personal-loan") {
      ...
      amountInput.max = String(Number(selected.debt || 0));
      ...
      if (submitButton) {
        submitButton.textContent = "Kart Borcunu Öde";
      }
    }
  ...
  function processPayment() {
    ...
    if (!id) {
      alert("Ödeme yapılacak kart veya krediyi seç.");
      return;
    }
    ...
    if (!selected) {
      alert("Seçilen borç kaydı bulunamadı.");
      return;
    }

    if (amount > Number(selected.debt || 0)) {
      alert("Ödeme tutarı kalan borçtan büyük olamaz.");
      return;
    }
    ...
    if (selected.type === "personal-loan") {
      ...
      if (Number(selected.remainingInstallments || 0) > 0) {
        selected.remainingInstallments =
          Number(selected.remainingInstallments || 0) - 1;
      }
      ...
      const oldCycle =
        selected.statementCycle ||
        cycleKey(selected.dueDate);
    ...
    sessionStorage.setItem("yf_v49_reopen_payment", "1");

    window.location.reload();
  }

  if (
    sessionStorage.getItem("yf_v49_reopen_payment") === "1"
  ) {
    sessionStorage.removeItem("yf_v49_reopen_payment");
  ...
  // -------------------------------------------------
  // ESKİ BORÇ SEÇİCİYİ SİL VE TEMİZİNİ KUR
  // -------------------------------------------------
  function rebuildDebtPicker() {
    $("debtPickerButton")?.remove();
    $("debtPickerLayer")?.remove();
    ...
    $("closeDebtPicker")?.addEventListener("click", closePicker);
  }

  function renderPickerRows() {
    const list = $("debtPickerList");
    if (!list) return;
    ...
    function appendGroup(title, items) {
      ...
      items.forEach(item => {
        ...
        row.dataset.debtId = String(item.id);
        ...
        row.innerHTML = `
          ...
          <span class="debt-picker-row-amount">
            <strong>${formatMoney(item.debt)}</strong>
            <small>
              ${item.type === "personal-loan" ? "Kalan kredi" : "Kart borcu"}
            </small>
          </span>
        `;
  ...
  function refreshPickerButton() {
    const text = $("debtPickerButtonText");
    const select = $("debtPaymentCard");
  ...
  function closePicker() {
    $("debtPickerLayer")?.classList.add("hidden");
    document.body.style.overflow = "";
  }
...
// =========================================
// YF v5.8 — MEVCUT TAKVİMİ FİNANS TAKVİMİNE YÜKSELT
// Yeni menü eklemez; çalışan Takvim menüsünü geliştirir.
// =========================================
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", event => {
    const calendarButton =
      event.target.closest('[data-page="calendarPage"]');
  ...
  function upgradeCalendarMenu() {
    const button =
      document.querySelector('[data-page="calendarPage"]');

    if (!button) return;

    const icon =
      button.querySelector(".side-menu-icon");
    ...
    const label =
      labels.find(span =>
        !span.classList.contains("side-menu-icon")
      );

    if (label) {
      label.textContent = "Finans Takvimi";
    }
  }

  function upgradeCalendarPage() {
    const page = $("calendarPage");
    if (!page) return;

    const eyebrow =
      page.querySelector(".top-header .eyebrow");

    const title =
      page.querySelector(".top-header h1");

    if (eyebrow) {
      eyebrow.textContent = "FİNANS MERKEZİ";
    }

    if (title) {
      title.textContent = "Finans Takvimi";
    }

    if (!$("financeCalendarSummaryV58")) {
      const existingSummary =
        page.querySelector(".calendar-summary-grid");
      ...
      summary.id =
        "financeCalendarSummaryV58";

      summary.className =
        "yf-v58-finance-summary";

      summary.innerHTML = `
        <article>
          <span>Planlanan</span>
          <strong id="yfV58Planned">₺0,00</strong>
          <small>Bu ay kart + kredi</small>
        </article>

        <article>
          <span>Ödenen</span>
          <strong id="yfV58Paid">₺0,00</strong>
          <small>Bu ay yapılan ödeme</small>
        </article>

        <article>
          <span>Kalan</span>
          <strong id="yfV58Remaining">₺0,00</strong>
          <small>Ay sonuna kadar</small>
        </article>
      `;

      const header =
        page.querySelector(".top-header");
    ...
    if (!$("yfV58FinanceInfo")) {
      ...
      info.className =
        "content-card yf-v58-finance-info";

      info.innerHTML = `
        <div class="section-heading">
          <div>
            <p class="eyebrow">TAKVİMDE NELER VAR?</p>
            <h2>Finans hareketleri</h2>
          </div>
        </div>

        <div class="yf-v58-info-grid">
          <span>💳 Kart son ödemeleri</span>
          <span>📄 Ekstre kesimleri</span>
          <span>🏦 Kredi taksitleri</span>
          <span>💵 Gelir ve maaş</span>
          <span>✓ Yapılan ödemeler</span>
        </div>
      `;

      const mainCard =
        page.querySelector(".calendar-main-card");
  ...
  function refreshFinanceSummary() {
    ...
    const target =
      parseTurkishMonthLabel(monthTitle);
  ...
  function parseTurkishMonthLabel(value) {
    const text =
      String(value || "")
        .trim()
        .toLocaleLowerCase("tr-TR");
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .yf-v58-finance-summary article {
        padding: 12px;
        border: 1px solid rgba(255,255,255,.06);
        border-radius: 14px;
        background: rgba(255,255,255,.025);
      ...
      .yf-v58-finance-summary span,
      .yf-v58-finance-summary strong,
      .yf-v58-finance-summary small {
        display: block;
      }
      ...
      .yf-v58-finance-summary article:first-child strong {
        color: #ffd073;
      }

      .yf-v58-finance-summary article:nth-child(2) strong {
        color: #4ce0aa;
      }
